...
export default {
  ...
  computed: {
    productsSum() {
      return this.$store.getters["productStore/getProductSum"];
    }
  },
  mounted() {
    this.productList = this.$store.getters["productStore/getProductList"];
  },
  beforeUpdate() {
  ...
  methods: {
    prodctDelete(index) {
      this.$store.dispatch("productStore/deleteOneProduct", index);
    },
    openInfo(productNo) {
