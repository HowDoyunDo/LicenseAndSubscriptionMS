        ...
        <template v-if="type === 'U'">
            ...
            <div class="inner">
                ...
                <table class="info-table">
                    <tr>
                        <th>이메일</th>
                        <td style="text-align:left"><input type="text" style="width:300px" v-model="useremail"/></td>
                    </tr>
                    <tr>
                        <th>사용자 이름</th>
                        <td style="text-align:left"><input type="text" style="width:300px" @input="username = $event.target.value"/></td>
                    </tr>
                    <tr>
                        <th>사용자 부서</th>
                        <td style="text-align:left"><input type="text" style="width:300px" v-model="userdept"/></td>
                    </tr>
                    <tr>
                        <th>마지막 로그인 날짜</th>
                        <td style="text-align:left"><input type="datetime-local" style="width:200px" v-model="startdate"/> ~ 
                ...
                <h4 style="color:gray">사용자 목록
                    <span v-if="this.delete === false">
                        <button style="margin: 10px; float:right" @click="deleteUsr()">삭제</button>
                    ...
                    <span v-if="this.delete === true">
                        <button style="margin: 10px; float:right" @click="deleteUsr()">취소</button>
                        <button style="width:100px; margin: 10px; float:right" @click="deleteCom()">삭제 완료</button>
                    </span>
                </h4>
        ...
        filteredList() {
            return Object.values(this.generalusers).filter(post => {
                return ( post.email
                    ...
                    && post.dept_name
                    .toLowerCase()
                    .includes(this.userdept.toLowerCase())
                    && ( this.startdate === '' ? true : (Date.parse(post.last_login) > Date.parse(this.startdate)) )
                    && ( this.enddate === '' ? true : (Date.parse(post.last_login) < Date.parse(this.enddate)) )
                )
            });
        },
    },
