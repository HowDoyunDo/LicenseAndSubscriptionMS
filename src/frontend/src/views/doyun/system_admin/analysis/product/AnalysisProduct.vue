<template>
  <div id="analysisSales" class="contents">
    ...
    <div class="inner">
      <p class="tip">
        통계 자료는 국세청 및 기타 제출용 자료로 사용이 불가능하며, 라이선스
        운영의 참고 자료로 이용하시기 바랍니다.
      </p>
      <div id="option">
        <span id="per">기간</span> <br /><br />
        <span v-bind:class="today_per" style="margin-left:7%" @click="selPer(1)"
          >오늘</span
        >
        ...
        <span v-bind:class="sixmonth_per" @click="selPer(6)">6개월</span>
        <br /><br />
        ...
        <div
          style="border:1px solid #ccc; padding:10px; width:10%; min-width:280px; margin-left:7%;"
        >
          <input
            type="number"
            style="width:100px"
            v-model="startPrice"
          />~<input type="number" style="width:100px" v-model="endPrice" />원
        </div>
      ...
      <span id="lasttab" v-bind:class="agent" @click="seltab(4)"
        >에이전트 정책</span
      >
  ...
  computed: {
    filteredList() {
      return Object.values(this.analysisList).filter((al) => {
        ...
        switch (this.poltab) {
          case 1:
            policy = al.visible === true;
            break;
          case 2:
            policy = al.visible === false;
            break;
          case 3:
            policy = al.standard === "U";
            break;
          case 4:
            policy = al.standard === "A";
            break;
        }
